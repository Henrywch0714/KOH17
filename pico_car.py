...
class ultrasonic():
    def __init__(self):
        self.Trig = Pin(0, Pin.OUT, pull=None)
        self.Echo = Pin(1, Pin.IN, pull=None)
    ...
    def Distance(self):
        pulse_time = self.Get_Duration_Time()
        if pulse_time <= 0:
            return 999
        cms = (pulse_time / 2) / 29.1
        # print ("distance_1 is:", cms)
        return int(cms)
...
class ir():
    ...
    def __init__(self):
        ...
        self._edge = 0
        self._value = None
        self._last_value = 254
        self._times = array.array('i',  (0 for _ in range(self._nedges + 1)))
    ...
    def decode(self, _):
        try:
            ...
            if width > 3000:  # 4.5ms space for normal data
                if self._edge < 68:  # Haven't received the correct number of edges
                ...
                raise RuntimeError(self.REPEAT if self._edge == 4 else self.BADREP)  # Treat REPEAT as error.
            else:
                raise RuntimeError(self.BADSTART)
        ...
        self._edge = 0
        self._parse_cmd(cmd, addr)
        
        
    def _parse_cmd(self, cmd, addr):
        if cmd >= self.REPEAT:
            if cmd != self.REPEAT:
                self._value = cmd
                self._last_value = cmd
            else:
                self._value = self._last_value
            # print("IR Read:", self._value, cmd)
        else:
            if cmd != self.REPEAT:
                self._value = None
            # print("IR Error:", cmd)
    ...
    def _cb_pin(self, line):
        ...
        if self._edge <= self._nedges:  # Allow 1 extra pulse to record overrun
            if not self._edge:  # First edge received
                self._tim.init(period=self._tblock , mode=Timer.ONE_SHOT, callback=self.decode)
